...
ELECTRICAL_DISCLAIMER = (
    "Schematic light/switch/socket point placement for visualization only, one set per room "
    "using common conventions. This is NOT a licensed electrician's or engineer's calculated "
    "design — real circuit design, wire gauge, and panel sizing depends on actual electrical "
    "load calculations and local code. Have a licensed electrician review before construction."
)
...
def compute_plumbing_overlay(rooms: list[dict[str, Any]], plot_length_ft: float, plot_width_ft: float) -> dict[str, Any]:
    ...
    empty fixture/pipe list rather than inventing placements."""
    ...
    for room in wet_rooms:
        ...
        fixtures.append({"room_name": room["name"], "x": center_x, "y": center_y})
    ...
    main_riser = {"x": riser_position["x"], "y": riser_position["y"], "edge": closest_edge}
...
def compute_electrical_overlay(rooms: list[dict[str, Any]], plot_length_ft: float, plot_width_ft: float) -> dict[str, Any]:
    """Every room gets one light point (room center), one switch point
    (schematically placed just inside the corner nearest the plot's own
    entry-facing edge, a common convention absent any real door-position
    data this app doesn't track), and socket points along two walls —
    all a visualization convention, not a calculated circuit design (see
    ELECTRICAL_DISCLAIMER)."""
    valid = _valid_rooms(rooms)
    ...
    for room in valid:
        center_x = room["x"] + room["length"] / 2
        center_y = room["y"] + room["width"] / 2
        lights.append({"room_name": room["name"], "x": center_x, "y": center_y})
        ...
        switches.append({"room_name": room["name"], "x": switch_x, "y": switch_y})

        # Sockets: two per room, along the room's two longer walls at
        # roughly the 1/3 and 2/3 points — a common, simple convention.
        sockets.append({"room_name": room["name"], "x": room["x"] + room["length"] * 0.33, "y": room["y"] + 0.3})
        sockets.append({"room_name": room["name"], "x": room["x"] + room["length"] * 0.67, "y": room["y"] + room["width"] - 0.3})
